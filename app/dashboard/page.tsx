...
export default async function DashboardPage() {
  ...
  const primaryRole =
    access.roles.find((role) => role.role === "tenant_owner")?.role ??
    access.roles.find((role) => role.role === "tenant_admin")?.role ??
    "sin rol";

  return (
    <div className="space-y-5">
      <PageHeader title="Dashboard" description="Gestion del club con sesion y roles reales desde Supabase." />
